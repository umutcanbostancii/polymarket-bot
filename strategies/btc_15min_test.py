"""
BTC 15-min TEST strategy — rapid signal-based entry.

Independent from BTC15MinStrategy. Uses candle + sentiment + aggregator scoring.
strategy field: "btc_15min_test"
"""
...
class BTC15MinTestStrategy(BaseStrategy):
    name = "btc_15min_test"
    ...
    def __init__(
        ...
        self._max_entry_price = getattr(config, "TEST_MAX_ENTRY_PRICE_15M", 0.85)
    ...
    async def scan(self):
        """One iteration of the test mode fast loop (15-min)."""
        self._scan_count += 1
        ...
        if not self._dt.step("risk_check", ok, reason if not ok else "OK"):
            if self._scan_count % 30 == 1:
                log.warning(f"[TEST-15m] Risk blocked: {reason}")
            return
        ...
        if not self._dt.step("market_discovery", bool(market), "Bulunamadi" if not market else "OK"):
            if self._scan_count % 15 == 1:
                log.info("[TEST-15m] No active BTC 15-min market found")
            return
        ...
        if market_id != self._current_market_id:
            ...
            log.info(
                f"[TEST-15m] New market: {market.get('question', '')[:80]} "
        ...
        # 5. WARMUP: 180s warmup (remaining > 720)
        max_remaining = getattr(self.cfg, "TEST_MAX_SECONDS_REMAINING_15M", 720)
        if remaining > max_remaining:
            self._dt.step("warmup", False, f"remaining={remaining:.0f}s > {max_remaining}")
            if self._scan_count % 30 == 1:
                log.info(f"[TEST-15m] Warmup: remaining={remaining:.0f}s > {max_remaining}")
            return
        self._dt.step("warmup", True, f"remaining={remaining:.0f}s")

        # 6. Hard floor: son 30s'de asla girme (15m marketler daha uzun)
        if remaining < 30:
            self._dt.step("signals", False, "hard floor <30s")
            return

        # 7. Collect rapid signals (with 300 1s candles)
        ...
        if remaining < dynamic_min:
            ...
            if self._scan_count % 15 == 1:
                log.info(
                    f"[TEST-15m] SKIP: too late for {strength} signal "
        ...
        if not self._dt.step("orderbook", bool(has_book),
            ...
            if self._scan_count % 30 == 1:
                log.info("[TEST-15m] SKIP: no CLOB orderbook")
            return
        ...
        if not self._dt.step("price_check", price <= self._max_entry_price,
            ...
            if self._scan_count % 30 == 1:
                log.info(f"[TEST-15m] SKIP: price too high {price:.4f}")
            return
        ...
        notes = (
            f"[TEST-15m] {result.action} score={result.composite_score:.4f} "
            f"agreement={result.agreement_ratio:.2f} "
        ...
        log.info(
            f"[TEST-15m] SIGNAL: {direction.upper()} "
            f"score={result.composite_score:.4f} "
            f"agreement={result.agreement_ratio:.2f} "
        ...
        if tid:
            ...
            log.info(f"[TEST-15m] Trade #{tid} placed: {direction.upper()} ${size:.2f}")
    ...
    async def _collect_rapid_signals(self, market: dict) -> list[SignalResult]:
        """Collect all signal sources rapidly (300 1s candles for 15m)."""
        signals = []

        try:
            # a. 1-second candle analysis (300 candles for 15m)
            candle_limit = getattr(self.cfg, "CANDLE_1S_LIMIT_15M", 300)
            klines = await self.binance.fetch_klines_1s(limit=candle_limit)
            if klines and len(klines) >= 10:
                candle_result = analyze_candles(klines, window="15m")
                if candle_result:
            ...
            # d. Momentum signal (longer window for 15m)
            momentum = self.binance.get_momentum(seconds=120)
            ...
            log.warning(f"[TEST-15m] Signal collection error: {e}")
    ...
    def _try_weight_update(self):
        ...
        try:
            ...
            if new_weights:
                ...
                log.info(f"[TEST-15m] Weights updated: {new_weights}")
        except Exception as e:
            log.warning(f"[TEST-15m] Weight update error: {e}")
    ...
    async def _schedule_resolution(
        ...
        try:
            ...
            if bet_correct is not None:
                ...
                log.info(
                    f"[TEST-15m] Trade #{trade_id} resolved[{resolution_source}]: "
            ...
            log.error(f"[TEST-15m] Resolution error for trade #{trade_id}: {e}")
